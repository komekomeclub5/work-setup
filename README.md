...
### 使い方（1案件あたり30秒）
1. `C:\web\_worksheets\` に `YYYY-MM-DD_案件番号.md` を新規作成
...
### コード連動スニペット（C:\web\ ワークスペース前提）

`C:\web\` を VSCode で開いている時、Gemini Code Assist が **案件コードと worksheet 両方を参照可能**になる。これを活かす3つ：

| prefix | 用途 | タイミング |
|--------|------|-----------|
| `casediff` | git diff から Phase 5「編集箇所」を自動抽出して worksheet に反映 | 編集後・コミット前 |
| `casecheck` | 最新 diff と worksheet Phase 6 を突き合わせ提出前チェック | push 直前 |
| `casecause` | 差し戻し指摘 + diff + worksheet を統合して原因調査 | 差し戻し受領時 |

#### 前提：gdiff で diff をファイル化

Gemini Code Assist は `git` コマンドを実行できないため、PowerShell ヘルパー `gdiff` で diff をファイルに書き出してから参照する。
...
その後 Snippet（`casediff` 等）を展開して Gemini に投げる。
溜まった diff ファイルは `gdiffclean` で削除。
