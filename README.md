...
### 使い方：覚えるのは `co` だけ（対話モード推奨）

worksheet ファイル内で `co` → Tab → スニペット展開 → Gemini Code Assist チャットに貼り付け → Enter。

副操縦士が「今の状況を1文字で教えてください（a〜e）」と聞いてくる。
あとは Gemini の質問に1問ずつ答えるだけで、新規案件・過去案件参照・提出前チェック・差し戻し対応・週次振り返りのすべてが進む。
