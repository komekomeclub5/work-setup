...
### ブラウザモード（Agent 権限がない場合のフォールバック）

会社で Gemini Code Assist の Agent 機能が使えない時用。ブラウザ Gemini に貼り付けて運用する。

#### セットアップ
- `windows/GEMINI.md` を `C:\web\GEMINI.md` にコピー（ブラウザモード手順が冒頭に記載される）
- `apply-vscode.bat` 実行 → `coa/coc/cod/cor/coe` snippet が反映

#### 1案件の操作
1. 案件 repo 内で `gctx` 実行 → `C:\web\_worksheets\_tmp\<repo>_context.md` がVSCodeで開く
2. `_co.md` 等のスクラッチに用途別 snippet を展開:
   - `coa` 新規案件 / `coc` 提出前チェック / `cod` 差し戻し対応 / `cor` ルール記録 / `coe` 週次振り返り
3. 依頼内容（or 差し戻し指摘）を該当欄に貼る
4. context ファイルから `Ctrl+A Ctrl+C` してコンテキスト欄に貼る
5. 全選択コピー → ブラウザ Gemini に貼り付け → Enter
6. 返ってきた作業シートを `_worksheets/YYYY-MM-DD_<案件番号>.md` として保存

#### 覚えること
- `gctx`（repo 内で実行）
- `coa/coc/cod/cor/coe`（用途別 snippet）

### Agent モード（推奨・Agent 権限がある場合）
